...
async function initMaps() {
  ...
  document
    .getElementById('left-recenter-button')
    .addEventListener('click', () => {
      leftMap.panTo(leftMapMarkerPosition)
    })

  document
    .getElementById('center-recenter-button')
    .addEventListener('click', () => {
      centerMap.panTo(centerMapMarkerPosition)
    })

  document
    .getElementById('right-recenter-button')
    .addEventListener('click', () => {
  ...
  document
    .getElementById('lock-right-marker')
    .addEventListener('change', () => {
      lockedMarker = 'right'
    })

  document
    .getElementById('lock-left-marker-to-bearing')
    .addEventListener('change', (event) => {
      leftMarkerLockedToBearing = event.target.checked
    })

  document
    .getElementById('lock-right-marker-to-bearing')
    .addEventListener('change', (event) => {
      rightMarkerLockedToBearing = event.target.checked
    })

  document
    .getElementById('lock-center-marker-to-bearing')
    .addEventListener('change', (event) => {
  ...
  leftMarker.addListener(
    'drag',
    debounce(() => markerMoved(leftMarker), 10)
  )
  centerMarker.addListener(
    'drag',
    debounce(() => markerMoved(centerMarker), 10)
  )
  rightMarker.addListener(
    'drag',
    debounce(() => markerMoved(rightMarker), 10)
  )
...
function centerMarkerMoved(centerMarker) {
  ...
  if (centerMarkerLockedToBearing) {
    ...
    const targetPosition =
      lockedMarker === 'right' ? leftMapMarkerPosition : rightMapMarkerPosition
    const oppositePosition =
      lockedMarker === 'right' ? rightMapMarkerPosition : leftMapMarkerPosition
...
function endpointMarkerMoved(movedMarker) {
  ...
  if (
    ...
    updatePreviewMarker(
      adjustedPosition,
      movedMarker === leftMarker ? leftMap : rightMap
    )
...
function updateMarkers(...markerCollections) {
  ...
  markerCollections.forEach((collection) => {
    const position =
      collection === leftMarkers
        ? leftMapMarkerPosition
        : collection === centerMarkers
        ? centerMapMarkerPosition
        : collection === rightMarkers
        ? rightMapMarkerPosition
        : null
...
function adjustPositionToGeodesicLine(
  draggedPosition,
  otherPosition1,
  otherPosition2
) {
...
function drawLines() {
  ...
  polylines.push(
    ...drawGeodesicLine(leftMapMarkerPosition, rightMapMarkerPosition, leftMap)
  )
  polylines.push(
    ...drawGeodesicLine(
      leftMapMarkerPosition,
  ...
  polylines.push(
    ...drawGeodesicLine(leftMapMarkerPosition, rightMapMarkerPosition, rightMap)
  )
}
